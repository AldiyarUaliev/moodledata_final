...
/**
 * Strings for component 'auth', language 'ru', version '3.10'.
 *
 * @package     auth
 * @category    string
...
$string['allowaccountssameemail'] = 'Разрешить учетные записи с одинаковыми адресами электронной почты';
$string['allowaccountssameemail_desc'] = 'При включенном параметре несколько учетных записей пользователей могут иметь одинаковый адрес электронной почты. Это может привести к проблемам безопасности и сохранения конфиденциальности, например, с письмом подтверждения изменения пароля.';
...
$string['auth_fieldlockfield'] = 'Блокировка значения ({$a})';
...
$string['auth_fieldmapping'] = 'Сопоставление данных ({$a})';
...
$string['auth_notconfigured'] = 'Метод аутентификации {$а} не настроен.';
...
$string['auth_passwordisexpired'] = 'Срок действия вашего пароля истек. Установите свой новый пароль.';
...
$string['auth_remove_user'] = 'Укажите, что делать с внутренней учетной записью пользователя во время массовой синхронизации, если пользователь будет удален из внешнего источника. Только заблокированные пользователи могут быть автоматически восстановлены, если они снова появятся во внешнем источнике.';
$string['auth_remove_user_key'] = 'При удалении пользователя во внешнем источнике';
$string['auth_sync_script'] = 'Синхронизация учетных записей пользователей';
$string['auth_sync_suspended'] = 'При включенном параметре атрибут блокировки будет использован для обновления состояния блокировки локальной учетной записи пользователя.';
$string['auth_sync_suspended_key'] = 'Синхронизировать состояние блокировки локальных пользователей';
$string['auth_updatelocal'] = 'Обновление локальной учетной записи';
$string['auth_updatelocal_expl'] = '<p><b>Обновление локальной учетной записи:</b> если параметр включен, то значения полей будут заменяться данными из внешнего источника аутентификации при каждом входе пользователя и при синхронизации пользователей. Поля, для которых включено обновление локальной учетной записи, следует заблокировать.</p>';
$string['auth_updatelocalfield'] = 'Обновление локальной учетной записи ({$a})';
$string['auth_updateremote'] = 'Обновление внешней учетной записи';
$string['auth_updateremote_expl'] = '<p><b>Обновление внешней учетной записи:</b> если параметр включен, то информация во внешнем источнике авторизации будет обновлена при обновлении учетных записей пользователей в Moodle. Поля, для которых включено обновление внешней учетной записи, следует разблокировать, чтобы было возможно их редактирование.</p>';
...
$string['auth_updateremotefield'] = 'Обновление внешней учетной записи ({$a})';
...
$string['authloginviaemail'] = 'Разрешить использовать адрес электронной почты в качестве логина';
$string['authloginviaemail_desc'] = 'Разрешить пользователям использовать как логин, так и адрес электронной почты (если он уникальный) для входа на сайт.';
...
$string['changepasswordhelp'] = 'URL страницы восстановления забытого пароля, который будет отправляться пользователю на электронную почту. Обратите внимание, что эта настройка не используется, если URL страницы восстановления был задан в общих настройках аутентификации.';
...
$string['createpasswordifneeded'] = 'Создавать пароль при необходимости и отправлять его по электронной почте';
...
Если у вас есть вопросы, обратитесь в службу поддержки по адресу: {$a->supportemail}';
...
$string['errorminpasswordnonalphanum'] = 'Пароль должен содержать не менее {$a} символов, не являющихся буквами и цифрами, например таких как *, - или #.';
...
$string['eventuserloginfailed'] = 'Неудачный вход пользователя';
...
$string['informminpasswordnonalphanum'] = 'не менее {$a} символов, не являющихся буквами и цифрами, например таких как *, - или #.';
...
$string['limitconcurrentlogins'] = 'Лимит одновременных подключений';
...
$string['ntlmsso_attempting'] = 'Попытка аутентификации через NTLM протокол...';
$string['ntlmsso_failed'] = 'Автоматический вход не удался, попробуйте обычную страницу входа...';
$string['ntlmsso_isdisabled'] = 'Протокол аутентификации NTLM SSO отключен.';
$string['passwordhandling'] = 'Обработка поля пароля';
...
$string['privacy:metadata:userpref:createpassword'] = 'Показывает, что пароль должен быть сгенерирован для пользователя';
$string['privacy:metadata:userpref:forcepasswordchange'] = 'Показывает, должен ли пользователь изменить свой пароль при входе в систему';
...
$string['recaptcha_help'] = 'CAPTCHA служит для предотвращения злоупотреблений со стороны автоматизированных программ. Следуйте инструкциям, чтобы подтвердить, что вы - человек. Это может быть поле выбора; символы, представленные в изображении или набор картинок для выбора.

Если Вы не можете разобрать изображение, Вы можете попробовать другую CAPTCHA или аудио-CAPTCHA.';
...
$string['settingmigrationmismatch'] = 'Выявлено несоответствие значений при исправлении имен настроек плагина! В плагине аутентификации «{$a->plugin}» параметр «{$a->setting}» под старым именем имел значение «{$a->legacy}», а под текущим именем - «{$a->current}». Последнее значение установлено как действующее, но вы должны проверить и подтвердить его.';
...
$string['testsettingsheading'] = 'Проверка параметров аутентификации - {$a}';
