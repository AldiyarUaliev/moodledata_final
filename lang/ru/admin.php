...
/**
 * Strings for component 'admin', language 'ru', version '3.10'.
 *
 * @package     admin
 * @category    string
...
$string['agedigitalconsentverification_desc'] = 'Включает проверку возраста цифровой дееспособности (дееспособности в отношении своих персональных данных) перед отображением страницы регистрации для самостоятельно регистрирующихся пользователей. Это защищает ваш сайт от несовершеннолетних, регистрирующихся без согласия родителей/опекунов. Не достигшим необходимого возраста при этом предоставляются <a target="_blank" href="{$a}">Контакты службы поддержки</a>.';
...
$string['cachetemplates'] = 'Кешировать шаблоны';
...
$string['configallowview'] = 'Выберите, какие роли пользователь будет видеть, сможет использовать в фильтрах и т. д. в зависимости от того, какие роли у него уже есть.';
...
$string['configcoursegraceperiodafter'] = 'Скрыть прошедшие курсы, продолжающиеся в течение многих дней, после даты окончания курса.';
$string['configcoursegraceperiodbefore'] = 'Скрыть будущие курсы для выполнения за много дней до даты начала курса.';
...
$string['configcustommenuitems'] = 'Здесь Вы можете настроить меню, отображаемое темами оформления. Каждая строка меню состоит из текста, URL (не обязательно), текста всплывающей подсказки (не обязательно) и разделенного запятыми списка кодов языков (не обязательно, для отображения элемента меню только пользователям с одним из указанных языков). Всё вышеперечисленное разделяется вертикальной чертой. Строки, начинающиеся с дефиса, будут отображаться как подменю предыдущего элемента высшего уровня. Для создания разделительной линии можно использовать символы  ###. Например:
<pre>
...
-Документация Moodle|https://docs.moodle.org|Moodle Docs
-German Moodle Docs|https://docs.moodle.org/de|Документация на немецком|de
###
Moodle.com|https://moodle.com/
</pre>';
$string['configcustomusermenuitems'] = 'Вы можете настроить содержимое меню пользователя (за исключением ссылки «Выход», которая добавляется автоматически). Каждая строка разделяется символами | и состоит из: 1) строки в форме «названиеязыка, названиекомпонента» или обычного текста, 2) адреса URL и 3) иконки в виде картинки или значка пикселя (в папке pix со структурой [имя_подпапки]/[имя_иконки], например, i/publish) или в виде URL. Также возможно создание разделительной линии путем добавления в нужном месте строки из одного или нескольких символов #.';
...
$string['configdefaulthomepage'] = 'Этот параметр определяет первую ссылку для пользователей, вошедших в систему';
...
$string['configenableactivitychooser'] = 'Окно выбора элемента курса — это диалоговое окно с кратким описанием каждого вида элемента курса и ресурса. При отключении этой функции будут отображаться отдельные раскрывающиеся списки для элементов курса и ресурсов.';
$string['configenableanalytics'] = 'Аналитические модели, такие как «Учащиеся с риском отчисления» или «Предстоящие мероприятия», могут генерировать прогнозы, отправлять аналитические уведомления и предлагать дальнейшие действия, такие как обмен сообщениями для пользователей.';
...
$string['configgeoipfile'] = 'Расположение двоичного файла данных GeoIP2 City. Этот файл не распространяется вместе с Moodle и должен быть получен отдельно, от <a href="https://www.maxmind.com/"> MaxMind</a>. Вы можете купить коммерческую версию или использовать бесплатную.<br />
...
$string['configlockrequestcategory'] = 'При включенном параметре пользователи с правом запроса новых курсов в контексте системы не смогут выбрать категорию при запросе нового курса. Альтернативный способ запретить пользователям запрашивать новый курс только в одной категории - применить возможность запрашивать новые курсы в контексте категории.';
...
$string['confirmcontextlock'] = '{$a->contextname} в настоящее время не заморожен. Замораживание делает его доступным только для чтения и не позволяет пользователям вносить изменения. Вы уверены, что хотите продолжить?';
$string['confirmcontextunlock'] = '{$a->contextname} в данный момент заморожено. Размораживание позволит пользователям вносить изменения. Вы уверены, что хотите продолжить?';
...
$string['coursecolorheading_desc'] = 'Любые курсы, не имеющие изображения, установленного в настройках курса, отображаются на информационной панели с шаблонной картой курса. Цвета, используемые в шаблоне, могут быть указаны ниже.';
...
$string['croninfrequent'] = 'Между двумя последними запусками сценария обслуживания cron было {$a->actual}, и он должен выполняться каждые {$a->expected}. Рекомендуется настроить его для более частого запуска.';
$string['cronremotepassword'] = 'Пароль cron для удаленного доступа';
$string['cronwarning'] = '<a href="{$a->url}">Скрипт admin/cron.php</a> не запускался {$a->actual} и должен запускаться каждые {$a->expected}';
$string['cronwarningcli'] = 'Скрипт <code>admin/cron.php</code> не запускался {$a->actual} и должен запускаться каждые {$a->expected}';
$string['cronwarningnever'] = 'Скрипт <code>admin/cli/cron.php</code> никогда не запускался, а должен запускаться каждые {$a->expected}.';
$string['cronwarningneverweb'] = 'Скрипт <a href="{$a->url}">admin/cron.php</a> никогда не запускался, а должен запускаться каждые {$a->expected}.';
...
$string['downloadcoursecontentallowed'] = 'Доступна функция загрузки содержимого курса';
$string['downloadcoursecontentallowed_desc'] = 'Доступна ли в курсах функция загрузки содержимого курса. Если доступна, то загрузка содержимого курса может быть включена / отключена с помощью параметра «Разрешить загрузку содержимого курса» в меню редактирования курса (значение по умолчанию для этого может быть установлено в <a href={$a} target="_blank"> Настройках курса по умолчанию</a>).';
...
$string['enableactivitychooser'] = 'Включить окно выбора элемента курса';
...
$string['environmentmariadbwrongdbtype'] = 'Неправильный <code>$CFG->dbtype</code>: измените в файле <code>config.php</code> параметр «<code>mysql</code>» на «<code>mariadb</code>».';
...
';
$string['geoipfile'] = 'База данных GeoLite2 City MaxMind';
...
$string['installsessionerror'] = 'Не удается инициализировать сессию PHP. Убедитесь, что Ваш браузер принимает cookies.';
...
$string['iplookupgeoplugin'] = 'В настоящее время для поиска географической информации  используется служба <a href="https://www.geoplugin.com">geoPlugin</a>. Для получения более точных результатов рекомендуется установить локальную копию базы данных MaxMind GeoLite.';
$string['iplookupinfo'] = 'По умолчанию для просмотра информации о местонахождении IP-адресов Moodle использует бесплатный сервер NetGeo (The Internet Geographic Database). К сожалению, эта база данных больше не поддерживается и может выдавать <em>совершенно неверные</em> данные. Рекомендуется установить локальную копию бесплатной базы данных GeoLite2 City компании MaxMind.<br />
...
$string['iplookupmaxmindnote'] = 'В этой программе используется база данных GeoLite2 компании MaxMind, доступная по адресу: <a href="https://www.maxmind.com/">http://www.maxmind.com/</a>.';
...
$string['linkcoursesections'] = 'Всегда отображать ссылки на разделы курса';
$string['linkcoursesections_help'] = 'Всегда пытаться отображать ссылки на разделы курса. Разделы курса обычно отображаются как ссылки, если формат курса предполагает один раздел на страницу. Если эта опция включена, то ссылки на разделы будет отображаться всегда.';
...
$string['logo'] = 'Логотип';
$string['logo_desc'] = 'Полный логотип, который будет использоваться в качестве оформления некоторыми темами (например, основными темами). Это изображение должно иметь довольно высокое разрешение, потому что оно будет уменьшено для использования (и кэшировано для производительности). Лучше использовать логотипы, у которых ширина больше высоты. Допустимые форматы: PNG и JPG.';
$string['logocompact'] = 'Компактный логотип';
$string['logocompact_desc'] = 'Компактная версия того же логотипа, что и выше, например, эмблема, иконка или значок. Изображение должно быть четким даже при небольших размерах. Допустимые форматы: PNG и JPG.';
$string['logossettings'] = 'Логотипы';
$string['logstorenotrequired'] = 'Взаимодействие с журналом событий не требуется';
$string['logstoressupported'] = 'Способы хранения журнала событий, которые поддерживают этот отчет';
...
$string['maintenancemodeisscheduled'] = 'Через {$a->min} минут {$a->sec} секунд сайт переходит в режим обслуживания';
...
$string['maxsizeperdownloadcoursefile_desc'] = 'Максимальный размер каждого файла при загрузке содержимого курса. Файлы, превышающие этот размер, будут исключены из загрузки.';
...
$string['messagingdeleteallnotificationsdelay'] = 'Удалить все уведомления';
...
$string['moodleapp_help'] = '<p> Бесплатное приложение Moodle позволяет пользователям получать доступ к своим курсам с мобильных устройств. Дополнительные функции приложения доступны с тарифным планом Pro или Premium. </p>
<a href="https://moodle.com/mobile-app"> Загрузите приложение Moodle </a> <br />
<a href="https://apps.moodle.com"> Портал приложений Moodle </a>';
$string['moodlebrandedapp'] = 'Фирменное приложение Moodle';
$string['moodlebrandedapp_help'] = '<p>Фирменное приложение Moodle имеет все функции нашего бесплатного мобильного приложения для Moodle в сочетании с вашим собственным пользовательским брендом. </p>
...
$string['moodleservices_help'] = 'Убедитесь, что вы получаете максимум от своей учебной среды Moodle, используя следующие сервисы Moodle:';
...
$string['navsortmycoursessort'] = 'Сортировать мои курсы';
...
$string['passwordchangelogout_desc'] = 'При включенном параметре после смены пароля все сессии браузера будут закрыты, кроме той, в которой и была произведена смена пароля. (Эта опция не влияет на смену пароля при пакетной загрузке пользователей.)';
...
$string['policysettings'] = 'Настройки политики';
...
$string['profilevisible_help'] = '* Не видно никому - для конфиденциальных пользовательских данных, доступных только администраторам.
...
$string['purgecachespage'] = 'Очистить кеши';
...
$string['purgeselectedcachesfinished'] = 'Выбранные кеши были очищены.';
...
$string['registermoodlenet'] = '<p>Мы бы хотели оставаться на связи и сообщать вам важные вещи о вашем сайте на Moodle!</p><p>Зарегистрировавшись:</p><ul><li>Вы будете одним из первых получать важные уведомления, такие как предупреждения о безопасности и новые выпуски Moodle.</li><li>Вы можете получить доступ и активировать мобильные push-уведомления с вашего сайта Moodle через наше бесплатное <a href="https://download.moodle.org/mobile/">приложение Moodle </a>.</li><li>Вы вносите свой вклад в нашу <a href="https://moodle.net/stats/">статистику Moodle</a> мирового сообщества, что помогает нам улучшать Moodle и сайты нашего сообщества.</li><li>Если вы хотите, сайт может быть включен в <a href="https://moodle.net/sites/">список зарегистрированных сайтов Moodle</a> в вашей стране.</li></ul>';
$string['registermoodleorg'] = 'Когда Вы зарегистрируете свой сайт';
...
$string['registrationwarningcontactadmin'] = 'Ваш сайт еще не зарегистрирован. Пожалуйста, сообщите об этом администратору.';
...
$string['sitesectionhelp'] = 'При включенном параметре верхний раздел будет отображаться на главной странице сайта.';
...
$string['task_adhoc_max_runtime_desc'] = 'Время выполнения разовой задачи, прежде чем она будет освобождена. Рекомендуется низкая продолжительность, так как нет ограничений на количество разовых задач в очереди. Если это число слишком велико и имеется большая очередь разовых задач, запланированные задачи могут выполняться нерегулярно.';
...
$string['task_logmode_all'] = 'Хранить в журнале результаты всех заданий';
...
$string['task_logretention_desc'] = 'Максимальный период, в течение которого должны храниться логи. Этот параметр взаимодействует с параметром «Хранение запусков»: применяется то, что будет достигнуто первым';
$string['task_logtostdout'] = 'Отобразить вывод логов';
$string['task_logtostdout_desc'] = 'Когда задания выполняются и выходные данные записываются, следует ли также отображать захваченные выходные данные при выполнении задачи.';
...
$string['taskbackupcleanup'] = 'Очистка таблиц и журналов резервного копирования';
$string['taskbadgescron'] = 'Награждение значками';
...
$string['taskdeletecachetext'] = 'Удаление записей кэша старого текста';
...
$string['taskquestionstatscleanupcron'] = 'Фоновая обработка для очистки кэша статистики вопросов';
...
$string['tasksessioncleanup'] = 'Очистка устаревших сессий';
...
$string['testoutgoingmailconf_message'] = 'Это тестовое сообщение, подтверждающее, что вы успешно настроили исходящую почту вашего сайта.';
...
$string['testoutgoingmailconf_subject'] = '{$a}: тестовое сообщение';
...
$string['unsupportedphpversion7'] = 'PHP версии 7 не поддерживается.';
$string['unsupportedphpversion71'] = 'PHP версии 7.1 не поддерживается.';
$string['unsupportedphpversion72'] = 'Версия PHP 7.2 не поддерживается.';
$string['unsupportedphpversion73'] = 'PHP версии 7.3 не поддерживается.';
$string['unsupportedphpversion74'] = 'Версия PHP 7.4 не поддерживается.';
$string['unsupportedphpversion80'] = 'Версия PHP 8.0 не поддерживается.';
...
$string['userfeedbackencouragement'] = '<p>Moodle 3.9 включает новую функцию, которая дает пользователям возможность отправлять отзывы о системе Moodle через внешний сайт опроса, расположенный на сервере штаб-квартиры Moodle. При этом никакая идентифицирующая пользователя информация не передается на сайт опроса.</p>
<p>Штаб-квартира Moodle стремится быть открытой и прозрачной в отношении своих практик сбора данных. Поэтому мы хотим удостовериться, что вы осведомлены об этом и сами контролируете этот функционал.</p>
...
$string['userfiltersdefault_desc'] = 'Выберите фильтры, которые должны отображаться на странице «Просмотр списка пользователей» и на странице «Массовые действия с пользователями». Удерживайте клавишу Ctrl, чтобы выбрать несколько элементов. Все фильтры, которые не выбраны, будут доступны при нажатии на ссылку «Показать больше».';
...
$string['xmlrpcrecommended'] = 'Расширение XMLRPC необходимо для веб-сервисов и Moodle networking';
