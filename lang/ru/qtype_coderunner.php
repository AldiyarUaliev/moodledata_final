...
/**
 * Strings for component 'qtype_coderunner', language 'ru', version '3.10'.
 *
...
$string['asolutionis'] = 'Авторское решение:';
$string['atleastonetest'] = 'Вы должны предоставить хотя бы один тестовый пример для этого вопроса.';
$string['attachmentoptions'] = 'Варианты вложения (экспериментальные)';
...
$string['badtemplateparams'] = 'Параметры шаблона должны быть либо пустыми, либо допустимой записью JSON';
$string['badtemplateparamsaftertwig'] = 'Ветвление параметров шаблона привело к недопустимому JSON: <pre>{$a}</pre>';
...
$string['feedback_help'] = 'Выберите "Установить в тесте", чтобы разрешить настройкам отзыва теста управлять отображением таблицы результатов, "Принудительно показать", чтобы обязательно показать таблицу результатов, и "Принудительно скрыть", чтобы обязательно скрыть ее.';
...
$string['goodemptyprecheck'] = 'Пройденный';
...
$string['gradingcontrols_help'] = 'Оценивание по умолчанию "точное совпадение" присуждает оценки только в том случае, если результат выполнения точно соответствует ожидаемому значению, определенному тестовым примером. Конечные пробелы удаляются из всех строк, а все конечные пустые строки удаляются до выполнения проверки на равенство.

Оценивание "почти-равенство" аналогично, за исключением того, что при этом также несколько пробелов и табуляций сворачиваются в один пробел, удаляются все пустые строки и строки преобразуются в нижний регистр.

Оценивание "регулярное выражение" использует поле "ожидаемое" как регулярное выражение и проверяет выходные данные, чтобы увидеть, можно ли найти совпадение с ожидаемым результатом в любом месте вывода.
Чтобы принудительно сопоставить весь вывод, начните и закончите регулярное выражение с \'\\A\' и \'\\Z\' соответственно. Сопоставление регулярных выражений использует МНОГОСТРОЧНЫЕ (MULTILINE) и точечные (DOTALL) параметры.

Опция "шаблонное оценивание" предполагает, что выходные данные программы на самом деле являются результатом оценки, то есть шаблон не проверяет *и оценивает* ответ студента. Единственным выходом из такой шаблонной программы должна быть запись в кодировке JSON.

Если шаблон является шаблоном для каждого теста (т.е. не комбинатором), строка JSON должна описывать строку таблицы результатов и содержать по крайней мере поле "дробная часть", которое умножается на TEST.mark, чтобы решить, сколько баллов присуждается тестовому примеру. Обычно он также должен содержать поле "получено", которое является значением, отображаемым в столбце "Получено" таблицы результатов.
Другие столбцы таблицы результатов (testcode, stdin, expected) также могут быть определены программой оценки по шаблону и будут использоваться вместо значений из тестового примера. Для примера, если выход из программы - строка <code>{"fraction":0.5, "got": "Половина ответов были правильные!"}</code>, половина баллов будет дано для этого конкретного тестового примера и в столбце  "Получено" будет отображать текст "Половина ответов были правильные!". Другие столбцы можно добавить в таблицу результатов, добавив дополнительные атрибуты к записи JSON, а также к полю вопросов Результирующие столбцы.

Если шаблон является комбинатором, то строка JSON, выводимая при шаблонном оценивании, должна снова содержать поле "дробная часть", на этот раз для общей оценки, и может содержать ноль или более "prologuehtml", "testresults", "epiloguehtml", "columnformats", "showoutputonly" и "showdifferences". Поля "prologuehtml" и "epiloguehtml" представляют собой html-код, который отображается соответственно до и после (необязательной) таблицы результатов. Поле
"testresults", если оно задано, представляет собой список списков, используемых для отображения какой-либо таблицы результатов. Первая строка - это строка заголовка столбца, а все остальные строки определяют тело таблицы. Существуют два специальных значения заголовка столбца: "iscorrect" и "ishidden". Столбцы "iscorrect" используются для отображения галочек или крестиков для значений строк 1 или 0 соответственно. Столбец "ishidden" на самом деле не отображается, но значения 0 или 1 в столбце можно использовать для включения и выключения видимости строк. Студенты не видят скрытых строк, но оценщики и другие сотрудники видят. Если предоставляется таблица "testresults", то также может быть предоставлено дополнительное поле "columnformats". Это должен быть список строк, по одной на столбец, исключая столбцы "iscorrect" и "ishidden". Строки задают формат, который будет использоваться для отображения значений ячеек; в настоящее время единственными поддерживаемыми форматами являются "%s" для обычного отображения строк (которые очищаются и обрамляются в элемент "pre") и "%h " для значения html, которое не должно обрабатываться перед отображением. Поле "showdifferences" включает отображение кнопки "Показать различия" после таблицы результатов, если доля присужденной оценки не равна 1.0. Поле "showoutputonly", если оно имеет значение true применяется, когда вопрос должен использоваться только для отображения выходных данных и, возможно, изображений из прогона без оценивания.';
...
- Дважды щелкните существующий узел, чтобы "пометить" его, например, как приемлемое состояние для конечных автоматов (FSMs). Дважды щелкните еще раз, чтобы снять пометку.
- Нажмите и перетащите, чтобы переместить узел.
- Alt+щелчок (или Ctrl+Alt+щелчок) и перетащите, чтобы переместить (суб)график.
- Shift+щелчок внутри одного узла и перетащите его в другой, чтобы создать ссылку.
- Shift+щелчок на пустое место, перетащите его в узел, чтобы создать начальную ссылку (только FSMs).
- Нажмите и перетащите ссылку, чтобы изменить ее кривую.
- Нажмите на ссылку/узел, чтобы отредактировать ее текст.
- Ввод _ с последующей цифрой делает эту цифру нижним индексом.
- Ввод \\epsilon создает символ epsilon (и аналогично для \\alpha, \\beta и т.д.).
- Нажмите на ссылку/узел, а затем нажмите клавишу Delete, чтобы удалить его (или функцию-delete на Mac).';
...
Режим штрафов по умолчанию может быть установлен системным администратором для всего сайта с помощью Администрирование сайта > Плагины > Типы вопросов > CodeRunner.
...
$string['missingorbadfraction'] = 'Плохая или отсутствующая дробь на выходе комбинатора оценивания. Вывод был: {$a->output}';
...
$string['missingprototypewhenrunning'] = 'Неработающий вопрос (отсутствует прототип \'{$a->crtype}\'). Невозможно запустить.';
...
$string['overallresult'] = 'Общий результат';
$string['pass'] = 'Пройти';
...
$string['questioncheckboxes_help'] = 'Чтобы настроить тип вопроса, например, отредактировать шаблоны вопросов или параметры песочницы, установите флажок "Настроить" и прочтите справку, доступную для новых видимых элементов формы, для получения дополнительной информации.
...
$string['resultstring-runtimeerror'] = 'Ошибка';
...
$string['syntax_errors'] = 'Синтаксическая ошибка(ы)';
...
$string['table_ui_missingparams'] = 'Таблица UI нуждается в параметрах шаблона table_num_columns и table_num_rows';
...
$string['twigcontrols'] = 'Управление наблюдением';
