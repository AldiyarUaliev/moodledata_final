...
/**
 * Strings for component 'lesson', language 'ru', version '3.10'.
 *
 * @package     lesson
 * @category    string
...
$string['customscoring_help'] = '<p>Этот параметр позволит Вам определить оценку за каждый ответ в вопросе. Ответы могут иметь отрицательные или положительные значения оценки. Для импортированных вопросов автоматически будет задано значение 1 для правильных ответов и 0 для неправильных, но это можно будет изменить после импорта.</p>';
...
$string['displayreview_help'] = 'При включении этого параметра после каждого неправильного ответа студенту будет предложено либо попробовать ответить на вопрос ещё раз (уже без получения баллов) либо просто продолжить лекцию. Если студент нажимает кнопку, чтобы перейти к другому вопросу, будет предложен выбранный (неправильный) ответ. По умолчанию переходы для неправильного ответа устанавливаются на «текущую страницу» и имеют оценку 0, поэтому рекомендуется установить переход с неправильным ответом на другую страницу, чтобы не запутать студентов.';
